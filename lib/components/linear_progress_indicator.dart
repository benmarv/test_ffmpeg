  ...
  void initState() {
    Future.delayed(Duration.zero, () {
      if (mounted) {
        ...
        final indicatorContext = _keyIndicator.currentContext;

        if (containerContext != null) {
          setState(() {
            _containerWidth = containerContext.size?.width ?? 0.0;
            _containerHeight = containerContext.size?.height ?? 0.0;
          });
        }

        if (indicatorContext != null) {
          setState(() {
            _indicatorWidth = indicatorContext.size?.width ?? 0.0;
            _indicatorHeight = indicatorContext.size?.height ?? 0.0;
          });
        }
      }
    });
