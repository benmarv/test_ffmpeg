...
class SadReaction extends StatefulWidget {
  const SadReaction({super.key});

  @override
  State<SadReaction> createState() => _SadReactionState();
}

class _SadReactionState extends State<SadReaction> {
  @override
  Widget build(BuildContext context) {
    return Consumer<ReactionListController>(
      builder: ((context, value, child) => value.isdata == false &&
              value.reactionlist5.isEmpty
          ? const Center(
              child: CircularProgressIndicator(),
            )
          : value.isdata != false && value.reactionlist5.isEmpty
              ? Center(
                  child: Column(
              ...
              : Padding(
                  ...
                  child: ListView.builder(
                    itemCount: value.reactionlist5.length,
                    shrinkWrap: true,
                    itemBuilder: (context, index) {
                      return ListTile(
                          ...
                          child: Container(
                            ...
                            child: CircleAvatar(
                              ...
                              backgroundImage: NetworkImage(
                                value.reactionlist5[index].avatar!,
                              ),
                            ),
                          ),
                        ),
                        title: Row(
                          ...
                          children: [
                            Text(
                              value.reactionlist5[index].username.toString(),
                              style: const TextStyle(fontSize: 15),
                            ),
                            4.sw,
                            value.reactionlist5[index].isVerified == "1"
                                ? verified()
